...
- Charging state shown with a pulsing dot
...
```js
// Battery widget: every 60 seconds
this.intervalId = setInterval(() => this.refresh(), 60000);

// Storage widget: every 4 hours
this.intervalId = setInterval(() => this.refresh(), 14400000);

// Countdown widget: every 6 hours
this.intervalId = setInterval(() => this.refresh(), 21600000);
```

Values are in milliseconds. Increase them to reduce background work, or decrease them for more frequent updates.
